...
$result = Invoke-Webrequest -Method Post `
          -Uri "$endpoint/translate?api-version=3.0&from=en&to=fr&to=it&to=zh-Hans&from=en&to=ja" `
          -Headers $headers `
          -Body $body 
...
$japanese = $analysis.translations[3] 
Write-Host ("Original Text: $text`nFrench Translation: $($french.text)`nItalian Translation: $($italian.text)`nChinese Translation: $($chinese.text)`nJapanese Translation: $($japanese.text)`n")
